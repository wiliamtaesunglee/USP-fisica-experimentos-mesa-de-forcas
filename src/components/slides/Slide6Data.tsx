...
export default function Slide6Data() {
  ...
  const theoCards = [
    ...
    { label: "Laranja", value: THEORETICAL.laranja, color: FORCE_COLORS.laranja },
  ];

  return (
    <SlideContainer id="dados">
      <div ref={ref} className="space-y-8 max-w-4xl mx-auto">
        <h2 className="s6-title text-3xl md:text-4xl font-bold text-center">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {theoCards.map((c) => (
            <div
              key={c.label}
              className="s6-theo flex items-center gap-3 px-5 py-3 rounded-xl bg-surface border"
            ...
            >
              ...
              <span className="text-xs text-muted">(teórico)</span>
            </div>
          ))}
        </div>

        {/* Data table */}
        <div className="s6-table p-4 rounded-xl bg-surface border border-surface-light">
          <p className="text-xs text-muted mb-3 uppercase tracking-wider font-medium">
            Amostra de 6 configurações (de 15 totais) — massas calculadas a partir dos ângulos de equilíbrio
          </p>
          <DataTable />
        </div>
      </div>
